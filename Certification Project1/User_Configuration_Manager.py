...
def add_setting(settings, pair):
    # Convert both key and value to lowercase for consistency
    key = pair[0].lower()
    value = pair[1].lower()

    # Check if the setting key already exists in the dictionary
    if key in settings:
        return f"Setting '{key}' already exists! Cannot add a new setting with this name."

    # Add the new key-value pair if it does not exist
    settings[key] = value
    return f"Setting '{key}' added with value '{value}' successfully!"

    
# Function to update an existing setting's value
def update_setting(settings, pair):
    # Convert both key and value to lowercase
    key = pair[0].lower()
    value = pair[1].lower()

    # If key exists, update its value in the dictionary
    if key in settings:
        settings[key] = value
...
# Function to delete a setting from the dictionary
def delete_setting(settings, key):
    # Convert key to lowercase
    key = key.lower()

    # If key exists, remove it using 'del'
    if key in settings:
        del settings[key]
...
# Function to display all current settings formatted as a string
def view_settings(settings):
    ...
    # Loop through each setting, capitalize key, and append to the result string
    for key, value in settings.items():
        result += f"\n{key.capitalize()}: {value}"

    # Add trailing newline character required by test suite
    result += "\n"
    return result


# Initialize the test_settings dictionary with sample data
test_settings = {
    'name': 'asad',
    'student': 'phd'
}
